-- @description Select next audible track at cursor position
-- @version 1.1
-- @about Identifies and selects the next audible track, reporting its name and peak info. We collect left and right peaks from track meters if playing, or read reapeak cache files for items if project is stopped. Muted tracks aren's aren't included.
...
-- 🡒 NEXT audible track
for _, info in ipairs(audible) do
  if info.index > current_idx then
    chosen_track = info.track
    break
  end
end

if not chosen_track then
  chosen_track = audible[1].track
  wrapped = true
end
