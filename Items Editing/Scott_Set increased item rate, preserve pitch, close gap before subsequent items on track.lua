-- @description Set increased item rate, preserve pitch, close gap before subsequent items on track
-- @version 1.0
...
for i = 1, #selected_by_track do
  ...
  for j = 1, #items do
    local changed, gap_closed = process_item(items[j], new_rate, true)
    if changed then
