-- @noindex
-- @description Move to end of previous silence on selected track

-- Implementation note:
-- Operates on the selected track, flowing backward across its items: it finds
-- where audio resumes after the nearest silence before the edit cursor
-- (continuing into earlier items if needed), moves the edit cursor there, and
-- selects the item it lands in. Detection reads REAPER's peak data
-- (GetMediaItemTake_Peaks, the .reapeaks cache). Peaks reflect the raw source level.
...
-- Scans [scan_lo, scan_hi] (project time, clamped to the item) backward for the
-- end (where audio resumes) of the nearest qualifying silence and returns its
-- project-time position, or nil. A silence abutting scan_hi is skipped (we
-- require audio, moving backward, before accepting one), so repeated runs keep
-- advancing and navigation flows on to earlier items.
local function find_in_item(item, scan_lo, scan_hi, threshold_amp, silence_duration)
  ...
  local silent_run = 0
  local end_bin = nil
  local seen_audio = false
  local hi_bin = total_bins
  while hi_bin > 0 do
    local lo_bin = math.max(0, hi_bin - CHUNK)
    local want = hi_bin - lo_bin
    local ret = reaper.GetMediaItemTake_Peaks(take, PEAKRATE,
      src_from + lo_bin * bin_dur, ch, want, 0, buf)
    ...
    for s = got - 1, 0, -1 do
      ...
      if amp < threshold_amp then
        if seen_audio then
          if silent_run == 0 then end_bin = lo_bin + s + 1 end
          silent_run = silent_run + 1
          if silent_run >= bins_needed then
            local found_src = src_from + end_bin * bin_dur
...
-- Backward movers always use the edit cursor (the play cursor runs ahead into
-- the audio, which would make "previous" return the silence you came from).
local cursor = reaper.GetCursorPosition()

-- Flow backward across the track: scan the cursor's item up to the cursor, then
-- each earlier item in full, until a silence end is found.
...
for i = #items, 1, -1 do
  ...
  if is < cursor then
    result = find_in_item(it, is, math.min(cursor, ie), threshold_amp,
      silence_duration)
    if result then
      found_item = it
      break
