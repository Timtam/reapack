-- @noindex
-- @description Move to beginning of next silence on selected track

-- Implementation note:
-- Operates on the selected track, flowing forward across its items: it finds the
-- next silence at/after the cursor (continuing into later items if needed), moves
-- the edit cursor to the start of that silence, and selects the item it lands in.
-- Detection reads REAPER's peak data (GetMediaItemTake_Peaks, the .reapeaks
-- cache), so there's no per-sample Lua loop. Peaks reflect the raw source level.
...
-- Scans [scan_lo, scan_hi] (project time, clamped to the item) for the start of
-- the next qualifying silence and returns its project-time position, or nil.
-- A silence is only accepted after audio is seen first, so a silence the cursor
-- (or item start) sits on is skipped and repeated runs keep advancing.
    ...
    for s = 0, got - 1 do
      ...
      if amp < threshold_amp then
        if seen_audio then
          ...
          if silent_bins >= bins_needed then
            local found_src = src_from + ((processed + s) - silent_bins + 1) * bin_dur
...
-- each later item from its start, until a silence is found.
