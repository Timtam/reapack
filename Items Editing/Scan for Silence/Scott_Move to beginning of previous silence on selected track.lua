-- @noindex
-- @description Move to beginning of previous silence on selected track

-- Implementation note:
-- Operates on the selected track, flowing backward across its items: it finds the
-- start of the nearest silence before the edit cursor (continuing into earlier
-- items if needed), moves the edit cursor there, and selects the item it lands in.
-- Detection reads REAPER's peak data (GetMediaItemTake_Peaks, the .reapeaks
-- cache). Peaks reflect the raw source level.
...
-- start of the nearest qualifying silence and returns its project-time position,
-- or nil. A silence reaching scan_lo without preceding audio returns nil, so
-- navigation flows on to the previous item.
    ...
    for s = got - 1, 0, -1 do
      ...
      if amp < threshold_amp then
        ...
        if silent_run >= bins_needed then
          local found_src = src_from + (lo_bin + s + 1) * bin_dur
...
-- each earlier item in full, until a silence is found.
