-- @noindex
-- @description Split at starts of silences (selected items, tracks, or time selection)

-- Implementation note:
-- Uses the same peak-cache silence detection as the Scan for Silence movers
-- (GetMediaItemTake_Peaks), but scans the whole item and splits at the START of
-- every qualifying silence. Threshold and duration come from the shared
-- Configure scan for silence settings.
...
-- Splits one item at the START of each qualifying silence falling within
...
local function process_item(item, scan_lo, scan_hi, threshold_amp, silence_duration)
  ...
  -- Build split positions from silence starts, snapping each to the nearest zero
  -- crossing and keeping only those strictly inside the item.
  local accessor = reaper.CreateTakeAudioAccessor(take)
  local positions = {}
  for _, r in ipairs(runs) do
    local pos = item_start + (r.start_src - start_offs) / playrate
...
local function do_work()
  ...
  reaper.Undo_EndBlock("Split items at starts of silences using Scan for Silence script", -1)
end
reaper.defer(do_work)
