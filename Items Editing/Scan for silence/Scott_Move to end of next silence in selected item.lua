-- @description Move to end of next silence in selected item
-- @version 1.0
...
-- FORWARD SCAN TO END OF SILENCE 
local function find_silence_forward_end(item, take, scan_start, scan_end, threshold_amp, silence_duration, window_size, hop_size)
  ...
  while b * hop_size <= max_offset do
    ...
    if peak < threshold_amp then
      ...
      if silent_samples >= silence_samples_needed then
        in_silence = true
        silence_end_offset = sample_offset + hop_size -- mark end of silence
      end
    else
      if in_silence and silence_end_offset then
...
local silence_offset = find_silence_forward_end(item, take, cursor, item_end, threshold_amp, silence_duration, window_size, hop_size)
...
reaper.Undo_EndBlock("Move cursor to end of next silence (OSARA)", -1)
