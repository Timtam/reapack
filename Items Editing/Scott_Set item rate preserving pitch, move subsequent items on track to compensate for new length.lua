-- @description Set item rate preserving pitch, move subsequent items on track to compensate for new length
-- @version 1.1
...
for i = 1, #selected_by_track do
  ...
  for j = 1, #items do
    local changed, amount_moved = process_item(items[j], new_rate, true)
    if changed then
