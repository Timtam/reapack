...
-- REVERSE SCAN TO END OF SILENCE 
local function find_silence_backward_end(item, take, scan_start, scan_end, threshold_amp, silence_duration, window_size, hop_size)
  ...
  while b >= 0 do
    ...
    if peak < threshold_amp then
      ...
      if not silence_started then
        silence_end_offset = sample_offset + hop_size -- first silent block encountered is the end
        silence_started = true
      end
    else
      if silent_samples >= silence_samples_needed and silence_end_offset then
        reaper.DestroyAudioAccessor(accessor)
        return (silence_end_offset + window_size) / sr
      end
      silent_samples = 0
      silence_started = false
      silence_end_offset = nil
    end

    b = b - 1
  end

  -- Handle silence at start
  if silent_samples >= silence_samples_needed and silence_end_offset then
    reaper.DestroyAudioAccessor(accessor)
    return silence_end_offset / sr
  end
...
local silence_offset = find_silence_backward_end(item, take, item_start, cursor, threshold_amp, silence_duration, window_size, hop_size)
...
reaper.Undo_EndBlock("Move cursor to end of previous silence (OSARA)", -1)
